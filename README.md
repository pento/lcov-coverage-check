...
The console log avoids repeating each failure twice: a per-file failure (new-file, changed-file) prints `FAIL: <path>` above its annotation, mirroring the format of the `PASS: <path> — …` lines, while a repo-wide failure (the overall ratchet) prints nothing before its annotation. The path stays at the end of the line with no trailing colon, so it can't be mistaken for a `path: message` line by a CI problem matcher (see [Key conventions](CLAUDE.md) for why that matters). The full failure message, with the affected file, still appears in the `Result: FAIL` list at the end of the log, plus the step summary and PR comment.
